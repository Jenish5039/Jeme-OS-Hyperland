...
apply_theme() {
    ...
    local current_wallpaper="$HOME/.cache/ml4w/hyprland-dotfiles/current_wallpaper"
    if [ ! -f "$current_wallpaper" ]; then
        echo "Error: Current wallpaper file not found at $current_wallpaper"
        return 1
    fi
    local wallpaper_path
    wallpaper_path="$(cat "$current_wallpaper")"
    if [ ! -f "$wallpaper_path" ]; then
        echo "Error: Wallpaper image file not found: $wallpaper_path"
        return 1
    fi
    ...
    if [[ "$THEME_PREF" == "1" || "$THEME_PREF" == "true" ]]; then
        ...
        "$MATUGEN_BIN" image "$wallpaper_path" --source-color-index 0 -m "dark"
        ...
        "$MATUGEN_BIN" image "$wallpaper_path" --source-color-index 0 -m "light"
...
stdbuf -oL inotifywait -m -q -e close_write,moved_to "$SETTINGS_DIR" | while read -r dir events filename; do
