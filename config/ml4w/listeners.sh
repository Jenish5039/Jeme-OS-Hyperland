...
start_listener() {
    ...
    local running_pids
    running_pids=$(pgrep -f "$base_name" | grep -v "^$$\$" || true)

    # Check if the script is already running
    if [ -n "$running_pids" ]; then
        echo "Listener '$script_name' is already running (PID: $(echo $running_pids | tr '\n' ' '))."
        return 0
    fi
...
stop_listener() {
    ...
    if [ -z "$running_pids" ]; then
        ...
        echo "Found PID(s) for '$script_name': $(echo $running_pids | tr '\n' ' '). Sending SIGTERM..."
        kill $running_pids 2>/dev/null || true
        # Give it a moment to terminate gracefully
        sleep 0.5
        local remaining
        remaining=$(pgrep -f "$base_name" | grep -v "^$$\$" || true)
        if [ -n "$remaining" ]; then
