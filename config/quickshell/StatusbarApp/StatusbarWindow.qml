...
PanelWindow {
    ...
    visible: barEnabled
    // Reserve space matching reservedHeight for clean window tiling below the bar.
    ...
    IpcHandler {
        ...
        // Bound to SUPER + SPACE. Toggles navigation mode on/off.
        function focus(): void {
            if (root.barExpanded) {
                root.barExpanded = false
            } else {
    ...
    Item {
        ...
        // Target expanded width: 94% of screen width by default (or percentage <= 1.0,
        // or pixel width > 1.0), clamped so it never exceeds screen bounds minus margins.
        property real targetExpandedWidth: {
            let pref = root.settings.pill.expandedWidth
            let w = (pref > 0 && pref <= 1.0) ? (root.width * pref)
                  : (pref > 1.0 ? pref : root.width * 0.94)
            return Math.min(root.width - 32, Math.max(w, contentWidth))
        }

        property real expandedWidth: targetExpandedWidth
        ...
        RectangularShadow {
            ...
            blur: 12
            color: Qt.rgba(Theme.shadow.r, Theme.shadow.g, Theme.shadow.b, 0.35)
        }
