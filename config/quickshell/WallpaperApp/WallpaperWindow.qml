...
PanelWindow {
    ...
    IpcHandler {
        ...
        function isThemingEnabled(): bool { return root.wallpaperTheming }
        function setTheming(enabled: bool): void {
            updateWallpaperTheming(enabled ? "true" : "false")
            wallpaperThemingSettingFileHandler.setText(enabled ? "true" : "false")
        }
    }
    ...
    property string wallpaperThemingSettingFile: Quickshell.env("HOME") + "/.config/ml4w/settings/wallpaper-theming"
    property bool wallpaperTheming: true

    FileView {
        id: wallpaperThemingSettingFileHandler
        path: Qt.url(root.wallpaperThemingSettingFile)
        blockLoading: true
        watchChanges: true
        onFileChanged: {
            this.reload();
            const settingValue = this.text().trim()
            console.log("Wallpaper theming setting changed on disk; attempting to update to \"" + settingValue + "\"")
            updateWallpaperTheming(settingValue)
        }
        onLoaded: {
            const settingValue = this.text().trim()
            console.log("Loading wallpaper theming setting \"" + settingValue + "\"")
            updateWallpaperTheming(settingValue)
        }
        onSaved: {
            const settingValue = this.text().trim()
            console.log("Wallpaper theming setting saved successfully; updating to \"" + settingValue + "\"")
            updateWallpaperTheming(this.text().trim())
        }
    }
    ...
    function updateWallpaperTheming(themingString): void {
        const cleaned = (themingString || "").toString().trim().toLowerCase();
        const isEnabled = !(cleaned === "0" || cleaned === "false" || cleaned === "off" || cleaned === "no");
        root.wallpaperTheming = isEnabled;
        if (typeof shouldUpdateTheming !== "undefined" && shouldUpdateTheming && shouldUpdateTheming.checked !== isEnabled) {
            shouldUpdateTheming.checked = isEnabled;
        }
    }
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                ML4WCheckBox {
                    ...
                    checked: root.wallpaperTheming
                    text: "Update theming from wallpaper"

                    onToggled: {
                        root.wallpaperTheming = checked
                        wallpaperThemingSettingFileHandler.setText(checked ? "true" : "false")
                    }
            ...
            GridView {
                ...
                ScrollBar.vertical: ScrollBar {
                     ...
                     contentItem: Rectangle {
                          ...
                          color: Theme.primary
                          opacity: wallpaperScrollBar.pressed
                                  ? 0.85
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (advancedOptions.visible) {
                                    ...
                                    options = `${outputParams}${positioningParams}`
                                }
                                const themingParams = root.wallpaperTheming
                                    ? ""
                                    : " --skip-theming"
                                options = `${options}${themingParams}`
